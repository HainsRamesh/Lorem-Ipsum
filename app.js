...
form.addEventListener("submit", function (e) {
  e.preventDefault();
  const userValue = parseInt(value.value);
  const randomNumber = Math.floor(Math.random() * text.length);
  if (isNaN(userValue) || userValue <= 0 || userValue > 9) {
    loremText.innerHTML = `<p>${text[randomNumber]}</p>`;
  } else {
    const slicedValue = text.slice(0, userValue);
    console.log(slicedValue);
    const result = slicedValue
      .map(function (para) {
        return `<p>${para}</P>`;
      })
      .join("");
